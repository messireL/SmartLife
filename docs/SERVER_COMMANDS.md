...
- после обновления запусти `./scripts/manage.sh sync`, чтобы в `last_status_payload` подтянулись свежие status/function codes устройства;
- открой карточку удлинителя: теперь там можно задать локальные имена каналов и увидеть диагностику, отдаёт ли Tuya поканальные метрики или только общие;
- если Tuya присылает только `add_ele`, `cur_power`, `cur_voltage`, `cur_current` без суффиксов каналов, SmartLife покажет честно только суммарную энергию по устройству.
