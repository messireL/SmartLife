...
## v0.11.35
- после `Pull and redeploy` полезно открыть `Устройства` и проверить `tdq`-розетки: теперь они должны показывать напряжение/ток/мощность/энергию из локальных DP `23/21/22/20`, а не висеть в неопределённом состоянии;
- для бойлера после выкладки нужно сделать обычный `sync`, чтобы новый display-path перечитал локальный raw payload и убрал ложный `fault`, если `dps[20] == 0`.
