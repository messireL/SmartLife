...
5. в каждом релизе обязательно обновляешь `docs/SERVER_COMMANDS.md`
...
- cloud-настройки Tuya теперь хранятся в PostgreSQL (`app_settings`), а не в `.env`;
- в `docs` поддерживается отдельный файл `SERVER_COMMANDS.md` с постоянно обновляемыми командами;
- контейнер `app` получает только явный whitelist SMARTLIFE_* переменных вместо полного `env_file`;
- `manage.sh` в начале очищает экран и удаляет остановленные ненужные Docker-контейнеры через `docker container prune -f`;
...
- v0.5.5 — исправление PostgreSQL 500 на главной/комнатах: список комнат переведён на безопасный DISTINCT через подзапрос без конфликтующего ORDER BY.

- v0.6.0 — cloud-настройки Tuya перенесены в PostgreSQL (`app_settings`), `manage.sh` получил `clear` + prune stopped containers в начале, а контейнер `app` теперь изолирован явным whitelist переменных вместо полного `env_file`.
