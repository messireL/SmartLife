...
- автообновление ключевых страниц по интервалу фоновой синхронизации;
- автоматическая очистка устройств провайдера demo, если проект работает не в demo-режиме;
...
3. фильтры по комнатам и типам устройств;  
4. автообновление через JS без полного reload страницы;  
5. multi-user и роли.
...
## Update v0.4.2
- Убрано пользовательское удаление обычных устройств из UI; оставлена только очистка demo-устройств.
- При старте non-demo проекта автоматически снимается старый флаг удаления с обычных устройств, если он остался от v0.4.1.
- Ручное удаление реальных устройств больше не предлагается; для них остаётся скрытие из интерфейса.
