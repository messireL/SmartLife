...
## Обновление v0.7.0
- Исправлен конфликт `COMPOSE_IGNORE_ORPHANS` vs `--remove-orphans`: `manage.sh` сам вычищает legacy-переменную из `.env` и окружения.
- Добавлен тариф электроэнергии в PostgreSQL и UI: стоимость теперь показывается на главной, в потреблении и в карточке устройства.
